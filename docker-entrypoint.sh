...
echo '[INFO] Waiting for mounting persistant volume (10s)';
sleep 10s

if [ -d "/usr/src/redmine" ]
then
	echo '[INFO] Persistant volume mounted'
	fileCount=$(find /usr/src/redmine -type f | wc -l)
	if [ $fileCount -gt 10 ]
         then
         echo "[INFO] Redmine files exists"
	else
		echo "[INFO] Copying redmine files to /usr/src/redmine";
		cp -r /usr/src/tmp/redmine/. /usr/src/redmine/.
		echo "[INFO] All redmine files copied to /usr/src/redmine";
	fi
else
	echo '[ERROR] /usr/src/redmine doesnt exists'
fi
